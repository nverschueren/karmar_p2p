...
function [p,t1,vel]=tintfreeze2d(p,t0,dt,nt,pmod,vel,vmod)
n=0; t=t0; par=p.u(p.nu+1:end); K=p.mat.K; Kx=p.mat.Kx; d1=par(1); del=par(6); dyy=par(7); 
%bK=[[par(1)*K 0*K];[0*K par(6)*K]];
LL=par(1)*p.mat.Kxx+par(7)*p.mat.Kyy; 
bK=[[LL, 0*LL]; [0*LL, del*LL]];
Lam=p.mat.M+dt*bK; [L,U,P,Q,R]=lu(Lam);
